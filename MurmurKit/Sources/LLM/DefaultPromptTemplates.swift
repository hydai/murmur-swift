...
/// Compile-time markdown templates for the five LLM prompts. Each one is the
/// fallback shown to users when no override file exists at
/// `{configDir}/prompts/{name}.md`.
///
/// Kept verbatim in sync with the repository's `prompts/*.md` source files.
enum DefaultPromptTemplates {
    static let postProcess = """
